...
// Dummy Student Data with approval status
const initialStudentData = [
  { id: 'STU001', name: 'Aarav Nair', class: '10A', age: 15, email: 'aaravnair@gmail.com', phone: '9876543210', status: 'Approval Request' },
  { id: 'STU002', name: 'Ananya Menon', class: '9B', age: 14, email: 'ananyamenon@gmail.com', phone: '9876543211', status: 'Verified' },
  { id: 'STU003', name: 'Rahul Das', class: '11C', age: 16, email: 'rahuldas@gmail.com', phone: '9876543212', status: 'Approval Request' },
  { id: 'STU004', name: 'Sneha Suresh', class: '8A', age: 13, email: 'snehasuresh@gmail.com', phone: '9876543213', status: 'Verified' },
  { id: 'STU005', name: 'Vishnu Raj', class: '12B', age: 17, email: 'vishnuraj@gmail.com', phone: '9876543214', status: 'Approval Request' },
  { id: 'STU006', name: 'Meera Joseph', class: '10B', age: 15, email: 'meerajoseph@gmail.com', phone: '9876543215', status: 'Verified' },
  { id: 'STU007', name: 'Arjun Kumar', class: '9A', age: 14, email: 'arjunkumar@gmail.com', phone: '9876543216', status: 'Approval Request' },
  { id: 'STU008', name: 'Diya Mathew', class: '11A', age: 16, email: 'diyamathew@gmail.com', phone: '9876543217', status: 'Verified' },
]
...
const ManageStudents = () => {
  ...
  const [filterStatus, setFilterStatus] = useState('Approval Request')
  ...
  const handleAddStudent = () => {
    ...
    if (id && name && studentClass && age && email && phone) {
      ...
      const newEntry = { id, name, class: studentClass, age, email, phone, status: 'Approval Request' }
      setStudentData([newEntry, ...studentData])
      setNewStudent({ id: '', name: '', class: '', age: '', email: '', phone: '' })
      setShowModal(false)
      setCurrentPage(1)
  ...
  const handleReject = (id) => {
    setStudentData(studentData.map((student) => (student.id === id ? { ...student, status: 'Rejected' } : student)))
  }

  const handleAccept = (id) => {
    setStudentData(studentData.map((student) => (student.id === id ? { ...student, status: 'Verified' } : student)))
  }
  ...
  return (
    <div className="container py-3 rounded bg-white">
      ...
      <div className="mb-3 d-flex gap-2">
        <button
          className={`btn btn-sm ${filterStatus === 'Approval Request' ? 'btn-warning' : 'btn-outline-warning'}`}
          onClick={() => {
            setFilterStatus('Approval Request')
            setCurrentPage(1)
          }}
        >
          Approval Request
        ...
        <button
          className={`btn btn-sm ${filterStatus === 'Verified' ? 'btn-warning' : 'btn-outline-warning'}`}
          onClick={() => {
      ...
      {currentStudents.length > 0 ? (
        <div className="table-responsive">
          <table className="table table-bordered align-middle text-center">
            ...
            <tbody>
              {currentStudents.map(({ id, name, class: studentClass, age, email, phone, status }) => (
                <tr key={id}>
                  <td>{id}</td>
                  <td className="text-start">{name}</td>
                  <td>{studentClass}</td>
                  ...
                  <td>
                    <div className="d-flex justify-content-center">
                      {status === 'Approval Request' && (
                        <>
                          <button className="btn btn-sm btn-outline-success me-2" onClick={() => handleAccept(id)}>
                            Accept
                          </button>
                          <button className="btn btn-sm btn-outline-danger" onClick={() => handleReject(id)}>
                            Reject
                          </button>
                        </>
                      )}
